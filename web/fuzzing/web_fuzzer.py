...
"""
 python3 web_fuzzer.py -u testasp.vulnweb.com -w fuzzdb/discovery/predictable-filepaths/login-file-locations/windows-asp.txt
"""
def main():
    ...
    pars.add_option('-w', '--wordlist', action="store", type="string",
                dest="wordlist", help="Custom wordlist", default=None)
...
def fuzz(url, CustomWordlist):
    ...
    if CustomWordlist:
        words = [w.strip() for w in open(str(CustomWordlist), "rb").
        readlines()]
    else:
        words = [w.strip() for w in open(wordlists["dict"], "rb").
        readlines()]
        try:
